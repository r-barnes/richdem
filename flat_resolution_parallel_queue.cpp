...
int BarnesStep(const float_2d &elevations, const char_2d &flowdirs, 
				int_2d &incrementations, tbb::concurrent_queue<grid_cell> edges, 
	...
	bool do_increment;
	bool first=true;
	grid_cell iteration_marker(-1,-1),c(0,0);

	while(!edges.empty() || first){
		for(int i=0;i<omp_get_max_threads();i++)
			edges.push(iteration_marker);
		loops++;
		first=false;

		#pragma omp parallel num_threads(omp_get_max_threads()) private(x, y, c, nx, ny, do_increment)
		while(edges.try_pop(c)){	//Only iteration marker is left in the end
			x=c.x;
			y=c.y;
...
int resolve_flats(float_2d &elevations, const char_2d &flowdirs){
	std::vector<grid_cell> llow_edges,lhigh_edges;	//TODO: Need estimate of size
	...
	BarnesStep(elevations, flowdirs, inc1, qlow_edges, flat_height, groups);
	BarnesStep(elevations, flowdirs, inc2, qhigh_edges, flat_height, groups);
