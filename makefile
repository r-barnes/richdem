compile: main.cpp
	mpic++ -o parallel_d8_accum.exe -g -O3 -DSHOW_STAMPS `gdal-config --cflags` `gdal-config --libs` main.cpp -I. -Irichdem/include -lgdal --std=c++11 -Wall #-Wextra #-fsanitize=undefined #-Wextra -Wconversion

compile_with_compression:
	mpic++ -o parallel_d8_accum.exe -g -O3 -DWITH_COMPRESSION `gdal-config --cflags` `gdal-config --libs` main.cpp -I. -lgdal --std=c++11 -Wall -lboost_iostreams -lz #-Wextra #-fsanitize=undefined #-Wextra -Wconversion
