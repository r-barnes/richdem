...
compile: main.cpp
	mpic++ -o parallel_flats.exe -g -O3 -DNDEBUG `gdal-config --cflags` `gdal-config --libs` main.cpp -I. -lgdal --std=c++11 -Wall #-Wextra #-fsanitize=undefined #-Wextra -Wconversion

compile_with_compression:
	mpic++ -o parallel_flats.exe -g -O3 -DNDEBUG -DWITH_COMPRESSION `gdal-config --cflags` `gdal-config --libs` main.cpp -I. -lgdal --std=c++11 -Wall -lboost_iostreams -lz #-Wextra #-fsanitize=undefined #-Wextra -Wconversion
...
debug: main.cpp
	mpic++ -o parallel_flats.exe -g     `gdal-config --cflags` `gdal-config --libs` main.cpp -I. -lgdal --std=c++11 -Wall
	#To run with debuggers: mpirun -n 2 xterm -hold  -e gdb -ex run --args ./parallel_flats.exe 0 beauford.tif
